...
"""
    ...
    async def review_backlog(self, backlog_content: str, project_id: int, backlog_id: int, feedback_id: int) -> str:
...
1. Provide constructive feedback with specific suggestions for improvement in Json format.  Do not provide any 
trivial feedback.
2. feedback on Stories related to performance testing, mobile, batch jobs, offline criteria, audit logs, localization, 
   multi-language, third party integration, infrastructure,  accessability, etc must be avoided.
3. If no feedback, then return None.  no extra text outside the document structure included. 
4. The suggestion arrays can have multiple suggestions. Above shown is just an example with 2 or 3 items. 
